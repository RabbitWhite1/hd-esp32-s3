...
| `web_ui.*` | LAN HTTP server (owns a `WebServer` on port 80): editable to-do checklist shown on the LCD, a form that sets the `claude_usage` credentials, and a Wi-Fi form that adds a network via `wifiAddNetwork` (tested-before-saved) and lists the saved ones | `webBegin`, `webHandle`, `webTodoCount` / `webTodoText` / `webTodoDone` |
